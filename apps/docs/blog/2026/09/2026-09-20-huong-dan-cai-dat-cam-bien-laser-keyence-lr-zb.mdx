...
**Quy trình thực hiện**:
  1. Tại màn hình *Run Mode*, nhấn và **giữ phím `[MODE]`**.
  2. Trong khi ngón tay vẫn đang giữ phím `[MODE]`, **nhấn phím `[DOWN]` liên tục 5 lần (`x5`)**.
  3. Màn hình sẽ chuyển sang trạng thái xác nhận và hiển thị mã **`ini`** (*Initialize*).
  4. Nhấn phím `[UP]` hoặc `[DOWN]` để chọn giữa **`ini`** (Tiến hành khôi phục) hoặc **`non`** (Hủy bỏ), sau đó nhấn phím `[MODE]` để xác nhận.

...
import { defsBlock, gridRect, trayPath, angleArc, dimHorizontal, dimVertical, dimAlong, labelPill, AMBER, STEEL, STEEL_L, CYAN } from '../utils/svgHelpers';
...
export default function Offset2Tab() {
  ...
  const svgContent = useMemo(() => {
    ...
    const barLen = 260;
    const cx = 170;
    const topY = 70, botY = topY + unitA;

    let svg = defsBlock() + gridRect(640, 380);

    const barLeft = cx - barLen / 2, barRight = cx + barLen / 2;
    const notchL = [cx - notchPx, topY], notchR = [cx + notchPx, topY], apex = [cx, botY];
    ...
    svg += angleArc(apex[0], apex[1], 30, -Math.PI / 2 - p, -Math.PI / 2 + p, CYAN, phi + '°+' + phi + '°');
    svg += dimHorizontal(cx, notchR[0], topY - 22, 'X = ' + fmt(X) + ' mm');
    svg += dimVertical(barLeft - 30, topY, botY, 'A = ' + fmt(A) + ' mm');
    svg += labelPill(cx, topY - 46, 'MẶT PHẲNG THÀNH MÁNG — CẮT KHẤC CHỮ V', STEEL_L);
    ...
    const f2 = [f1[0] + foldLegLen * Math.cos(bendAngleTotal), fy + 80 - foldLegLen * Math.sin(bendAngleTotal)];
    svg += trayPath([f0, f1, f2], STEEL, 16);
    svg += `<circle cx="${f1[0]}" cy="${f1[1]}" r="4" fill="${AMBER}"/>`;
    svg += dimAlong(f0[0], f0[1] + 26, f1[0], f1[1] + 26, 'L2 trong = ' + fmt(L2a) + ' mm', CYAN, 2);
    svg += dimAlong(
      f1[0] + (f2[0] - f1[0]) * 0.15, f1[1] + (f2[1] - f1[1]) * 0.15 + 18,
      f2[0] - (f2[0] - f1[0]) * 0.1, f2[1] - (f2[1] - f1[1]) * 0.1 + 18,
      'L2 ngoài = ' + fmt(L2b) + ' mm', AMBER, 2
    );
    svg += labelPill(f1[0], fy + 40, 'SAU KHI GẤP LẠI (2 KHÚC ' + fmt(phi) + '°)', STEEL_L);
    return svg;
  }, [A, phi, result]);
    ...
    <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4">
      <div className="space-y-5 bg-card border border-border rounded-lg p-4">
        ...
        <div className="space-y-2">
          ...
          <Input type="number" value={A} min={10} max={600} step={5}
        ...
        <div className="space-y-2">
          ...
          <Slider value={[phi]} min={5} max={40} step={0.5} onValueChange={(v) => setPhi(Array.isArray(v) ? v[0] : v)} />
        </div>
        <p className="text-xs text-yellow-600 border-t border-dashed border-yellow-600/30 pt-3">
          Công thức khấc cắt X = A·tan(φ) đã được kiểm chứng khớp với bảng tra gốc. Riêng 2 giá trị mép cắt trong/ngoài (L2) là suy ra theo hình học nhất quán — nên đối chiếu với bảng tra hoặc đo thử trên vật liệu thật trước khi cắt hàng loạt.
        </p>
      </div>
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-4 flex-wrap text-[11px] text-muted-foreground px-0.5">
          <span className="inline-flex items-center gap-1.5"><span className="w-4 h-0.5 bg-gray-400 inline-block" />Thành máng</span>
          <span className="inline-flex items-center gap-1.5"><span className="w-4 h-0.5 inline-block" style={{ background: 'repeating-linear-gradient(90deg,#ff7a6b 0 5px,transparent 5px 9px)' }} />Đường cắt khấc chữ V</span>
        </div>

        <FormulaBar html={`<b>X</b> = A·tan(φ) = ${fmt(A)}×tan(${fmt(phi)}°) = <b>${fmt(result.X)} mm</b>`} />
        <ReadoutPanel items={[

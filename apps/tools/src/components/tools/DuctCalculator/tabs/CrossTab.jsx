...
export default function CrossTab() {
  ...
  const svgContent = useMemo(() => {
    ...
    function branch(sign) {
      ...
      const bdx = Math.cos(a), bdy = -Math.sin(a) * sign;
      const branchLen = 110;
      const b1 = { x: nx1, y: edgeY }, b2 = { x: nx2, y: edgeY };
    ...
    svg += branch(1);
    svg += branch(-1);
    svg += dimHorizontal(cx - halfN, cx + halfN, mainY - mainH / 2 - 26, 'khấc = ' + fmt(notch) + ' mm');
    ...
    <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4">
      ...
      <div className="space-y-3">
        ...
        <div className="flex gap-4 flex-wrap text-[11px] text-muted-foreground px-0.5">
          <span className="inline-flex items-center gap-1.5"><span className="w-4 h-0.5 bg-gray-400 inline-block" />Máng chính / 2 nhánh</span>
          <span className="inline-flex items-center gap-1.5"><span className="w-4 h-0.5 inline-block" style={{ background: 'repeating-linear-gradient(90deg,#ff7a6b 0 5px,transparent 5px 9px)' }} />Khấc cắt 2 bên</span>
        </div>

        <FormulaBar html={`<b>Khấc</b> = W2 / sin(α) = ${fmt(W2)} / sin(${fmt(alpha)}°) = <b>${fmt(result.notch)} mm</b> <span style="color:${STEEL_L}">(áp dụng cho cả 2 bên)</span>`} />
        <ReadoutPanel items={[
          { label: 'Chiều dài khấc mỗi bên', value: fmt(result.notch), unit: 'mm' },

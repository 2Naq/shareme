...
export default function ElbowTab() {
  ...
  const svgContent = useMemo(() => {
    ...
    const far1_top = {
      x: p1_left_pt.x - fwd1.x * legLen,
      y: p1_left_pt.y - fwd1.y * legLen,
    };
    const far1_bot = {
      x: p1_right_pt.x - fwd1.x * legLen,
      y: p1_right_pt.y - fwd1.y * legLen,
    };
    const far2_top = {
      x: p2_left_pt.x + dir2.x * legLen,
      y: p2_left_pt.y + dir2.y * legLen,
    };
    const far2_bot = {
      x: p2_right_pt.x + dir2.x * legLen,
      y: p2_right_pt.y + dir2.y * legLen,
    };

    let svg =
      defsBlock() +
      `<rect width="640" height="380" fill="#0d1418"/><rect width="640" height="380" fill="url(#grid)"/>`;
    ...
    svg += labelPill(
      (outer.x + inner.x) / 2 + (dir2.x - fwd1.x) * 22,
    ...
    <div className="grid grid-cols-1 gap-4 md:grid-cols-[280px_1fr]">
      <div className="bg-card border-border space-y-5 rounded-lg border p-4">
        ...
        <p className="text-muted-foreground border-border border-t border-dashed pt-3 text-xs">
          Co ngang (elbow) đổi hướng máng trong cùng một mặt phẳng. Đường cắt
          vát (miter) chia đôi góc rẽ Θ. Θ = 90° là co vuông góc phổ biến nhất.
        </p>
      </div>
      ...
      <div className="space-y-3">
        <div className="flex gap-1.5">
          <button
            onClick={() => setView("3d")}
            className={`rounded border px-3 py-1.5 font-mono text-xs transition-colors ${view === "3d" ? "border-amber-400 bg-amber-400/10 text-amber-400" : "border-border text-muted-foreground hover:text-foreground"}`}
          >
            3D mô phỏng
          </button>
          <button
            onClick={() => setView("2d")}
            className={`rounded border px-3 py-1.5 font-mono text-xs transition-colors ${view === "2d" ? "border-amber-400 bg-amber-400/10 text-amber-400" : "border-border text-muted-foreground hover:text-foreground"}`}
          >
            2D mặt cắt
          </button>
          <button
        ...
        <div className="text-muted-foreground flex flex-wrap gap-4 px-0.5 text-[11px]">
          <span className="inline-flex items-center gap-1.5">
            <span className="inline-block h-0.5 w-4 bg-gray-400" />2 đoạn máng
          </span>
          <span className="inline-flex items-center gap-1.5">
            <span
        ...
        <FormulaBar
          html={`<b>L</b> = A / cos(Θ/2) = ${fmt(A)} / cos(${fmt(theta / 2)}°) = <b>${fmt(result.L)} mm</b>`}
        />
        <ReadoutPanel
          items={[
            {
              label: "Chiều dài đường cắt vát (L)",
              value: fmt(result.L),
              unit: "mm",
            },
            {
              label: "Góc cắt mỗi bên (Θ/2)",
              value: fmt(theta / 2),
              unit: "°",
            },
            {
              label: "Chênh lệch so với A",
              value: fmt(result.L - A),
              unit: "mm",
            },
          ]}
        />

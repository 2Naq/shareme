...
export default function Offset1Tab() {
  ...
  const svgContent = useMemo(() => {
    ...
    svg += dimAlong(
      p1[0] + (p2[0] - p1[0]) * 0.18,
      p1[1] - 20 + (p2[1] - p1[1]) * 0.18 - 14,
      p2[0] - (p2[0] - p1[0]) * 0.18,
      p2[1] - 20 - (p2[1] - p1[1]) * 0.18 - 14,
      "L = " + fmt(L) + " mm",
      AMBER,
    );
    ...
    svg += dimHorizontal(
      ...
      p2[0],
      baseY - 125,
      "chạy ngang = " + fmt(run) + " mm",
      STEEL_L,
    );
    return svg;
  }, [A, beta, result]);

  const flatData = useMemo(() => flatPattern_offset1(A, beta), [A, beta]);
    ...
    <div className="grid grid-cols-1 md:grid-cols-[280px_1fr] gap-4">
      ...
      <div className="space-y-3">
        ...
        <ReadoutPanel
          items={[
            {
              label: "Chiều dài đoạn chéo (L)",
              value: fmt(result.L),
              unit: "mm",
            },
            {
              label: "Chạy ngang (hình chiếu)",
              value: fmt(result.run),
              unit: "mm",
            },

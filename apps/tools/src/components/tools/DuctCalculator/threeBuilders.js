...
export function makeUSegment(THREE, length, width, wallH) {
  ...
  const right = new THREE.Mesh(
    new THREE.BoxGeometry(t, wallH, length),
    matDark,
  );
...
export function build3D_offset1(THREE, objGroup, setTarget, A, beta) {
  ...
  const seg1 = makeUSegment(THREE, p1.distanceTo(p0), width, wallH);
  alignSegment(THREE, seg1, p0, p1);
  const seg2 = makeUSegment(THREE, p2.distanceTo(p1), width, wallH);
  alignSegment(THREE, seg2, p1, p2);

  // Shift top horizontal segment (seg3) so its top edge meets seg2's top edge (hinge at top)
  const p2_shifted = new THREE.Vector3(
    p2.x - wallH * Math.sin(b),
    rise - wallH * (1 - Math.cos(b)),
    0,
  );
  const p3_shifted = new THREE.Vector3(
    p3.x - wallH * Math.sin(b),
    p2_shifted.y,
    0,
  );
  const seg3 = makeUSegment(
    THREE,
    p3_shifted.distanceTo(p2_shifted),
    width,
    wallH,
  );
  alignSegment(THREE, seg3, p2_shifted, p3_shifted);
  objGroup.add(seg1, seg2, seg3);

  const dirDiag = new THREE.Vector3().subVectors(p2, p1).normalize();
  const dirH = new THREE.Vector3(1, 0, 0);
  objGroup.add(
    cutMarker(THREE, p1, dirDiag.clone().multiplyScalar(0.4), width, wallH),
  );
  objGroup.add(
    cutMarker(
      THREE,
      p2_shifted,
      dirH.clone().multiplyScalar(0.4),
      width,
      wallH,
    ),
  );
  ...
  const p1_top_seg2_left = new THREE.Vector3(
    p1.x - wallH * Math.sin(b),
    wallH * Math.cos(b),
    -halfW,
  );
  const p1_top_seg2_right = new THREE.Vector3(
    p1.x - wallH * Math.sin(b),
    wallH * Math.cos(b),
    halfW,
  );
  ...
  objGroup.add(
    cutTriangleMesh(THREE, p1_bot_left, p1_top_left, p1_top_seg2_left),
  );
  objGroup.add(
    cutTriangleEdges(THREE, p1_bot_left, p1_top_left, p1_top_seg2_left),
  );

  // Right wall V-notch (apex at bottom p1)
  const p1_bot_right = new THREE.Vector3(p1.x, 0, halfW);
  objGroup.add(
    cutTriangleMesh(THREE, p1_bot_right, p1_top_right, p1_top_seg2_right),
  );
  objGroup.add(
    cutTriangleEdges(THREE, p1_bot_right, p1_top_right, p1_top_seg2_right),
  );

  // --- Cut Visualization at Junction p2 (Bend DOWN: cut from bottom up) ---
  // Hinge at top edge: p2_top
  const p2_top_left = new THREE.Vector3(
    p2.x - wallH * Math.sin(b),
    rise + wallH * Math.cos(b),
    -halfW,
  );
  const p2_top_right = new THREE.Vector3(
    p2.x - wallH * Math.sin(b),
    rise + wallH * Math.cos(b),
    halfW,
  );
  ...
  objGroup.add(
    cutTriangleEdges(THREE, p2_top_left, p2_bot1_left, p2_bot2_left),
  );
  ...
  objGroup.add(
    cutTriangleMesh(THREE, p2_top_right, p2_bot1_right, p2_bot2_right),
  );
  objGroup.add(
    cutTriangleEdges(THREE, p2_top_right, p2_bot1_right, p2_bot2_right),
  );
  ...
  setTarget(
    new THREE.Vector3((p0.x + p3.x) / 2, rise / 2, 0),
    Math.max(4.5, legLen * 2 + run + 1.5),
  );
}
...
export function build3D_offset2(THREE, objGroup, setTarget, A, phi) {
  ...
  const seg1 = makeUSegment(THREE, barLen, width, wallH);
  alignSegment(THREE, seg1, q0, q1);
  objGroup.add(seg1);
  ...
  const q2 = new THREE.Vector3().addVectors(
    q1,
    dir2.clone().multiplyScalar(barLen),
  );
  const seg2 = makeUSegment(THREE, barLen, width, wallH);
  alignSegment(THREE, seg2, q1, q2);
  objGroup.add(seg2);
  ...
  objGroup.add(
    cutTriangleMesh(
      THREE,
      topL,
      new THREE.Vector3(q1.x, wallH, -width / 2 + 0.001),
      apexL,
    ),
  );
  objGroup.add(
  ...
  const hingeMat = new THREE.LineBasicMaterial({
    color: AMBER_HEX,
    linewidth: 2,
  });
...
export function build3D_elbow(THREE, objGroup, setTarget, A, theta) {
  ...
  const outer = to3(outer2),
    inner = to3(inner2);

  const far1_top2 = new THREE.Vector2(
    p1L.x - fwd1.x * legLen,
    p1L.y - fwd1.y * legLen,
  );
  const far1_bot2 = new THREE.Vector2(
    p1R.x - fwd1.x * legLen,
    p1R.y - fwd1.y * legLen,
  );
  const far2_top2 = new THREE.Vector2(
    p2L.x + dir2.x * legLen,
    p2L.y + dir2.y * legLen,
  );
  const far2_bot2 = new THREE.Vector2(
    p2R.x + dir2.x * legLen,
    p2R.y + dir2.y * legLen,
  );
  const far1Mid = to3(
    new THREE.Vector2(
      (far1_top2.x + far1_bot2.x) / 2,
      (far1_top2.y + far1_bot2.y) / 2,
    ),
  );
  const far2Mid = to3(
    new THREE.Vector2(
      (far2_top2.x + far2_bot2.x) / 2,
      (far2_top2.y + far2_bot2.y) / 2,
    ),
  );
  const jointMid = new THREE.Vector3(
    (outer.x + inner.x) / 2,
    0,
    (outer.z + inner.z) / 2,
  );
  ...
  const outerTop = outer.clone().setY(wallH),
    innerTop = inner.clone().setY(wallH);
...
export function build3D_tee(THREE, objGroup, setTarget, W1, W2, alpha) {
  ...
  const m0 = new THREE.Vector3(-mainLen / 2, 0, 0),
    m1 = new THREE.Vector3(mainLen / 2, 0, 0);
  const main = makeUSegment(THREE, mainLen, w1, wallH);
  alignSegment(THREE, main, m0, m1);
  objGroup.add(main);
  ...
  const end = new THREE.Vector3().addVectors(
    start,
    dirBranch.clone().multiplyScalar(branchLen),
  );
  ...
  const perpBranch = new THREE.Vector3(
    dirBranch.z,
    0,
    -dirBranch.x,
  ).normalize();
...
export function build3D_cross(THREE, objGroup, setTarget, W1, W2, alpha) {
  ...
  function addBranch(sign) {
    ...
    const end = new THREE.Vector3().addVectors(
      start,
      dir.clone().multiplyScalar(branchLen),
    );
    const branch = makeUSegment(THREE, branchLen, w2, wallH * 0.85);
    ...
    const nc3 = c2
      .clone()
      .add(perpBranch.clone().multiplyScalar(sign * notchW));
    const nc4 = c1
      .clone()
      .add(perpBranch.clone().multiplyScalar(sign * notchW));
    objGroup.add(cutQuadMesh(THREE, nc1, nc2, nc3, nc4));

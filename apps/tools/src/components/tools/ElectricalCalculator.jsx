...
const CB_SIZES = [6, 10, 16, 20, 25, 32, 40, 50, 63, 80, 100, 125, 160, 200, 250, 320, 400, 500, 630, 800, 1000, 1250, 1600];
...
export default function ElectricalCalculator() {
  ...
  const { resultI, resultP, resultU, formula, cbSuggested, wireSuggested } = useMemo(() => {
    ...
    let selectedCb = CB_SIZES.find(size => size >= targetCb) || CB_SIZES[CB_SIZES.length - 1];

    // Gợi ý dây dẫn
    let selectedWire = WIRE_SIZES.find(wire => wire.maxI >= targetCb) || WIRE_SIZES[WIRE_SIZES.length - 1];
    ...
    <div className="mx-auto space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">
          Công cụ tính toán công suất, dòng điện, điện áp và gợi ý chọn thiết bị CB, dây dẫn.
        </p>
      </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6'>
        <Card className="md:col-span-2">
          <CardHeader>
            ...
            <CardDescription>Nhập các thông số cần thiết để tính toán.</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-6">
              <div className="space-y-2">
                <Label>Hệ thống điện</Label>
                <Select value={systemType} onValueChange={(val) => {
                  setSystemType(val);
        ...
        <div className="space-y-6">
          <Card className="bg-primary/5 border-primary/20">
            ...
            <CardContent className="space-y-6">
              <div>
                <Label className="text-xs text-primary uppercase tracking-wider">Công thức áp dụng</Label>
              ...
              <div>
                ...
                <div className="mt-1 font-mono text-4xl font-black text-primary">
                  {calcTarget === 'I' && <>{resultI} <span className="text-2xl">A</span></>}
                  {calcTarget === 'P' && <>{resultP} <span className="text-2xl">kW</span></>}
                  {calcTarget === 'U' && <>{resultU} <span className="text-2xl">V</span></>}
                </div>
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Tham khảo chọn CB và cáp điện (Cu/PVC)</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              ...
              <div className="flex justify-between items-center pb-2 border-b">
                <span className="text-muted-foreground">Tiết diện dây dẫn:</span>
                <Badge variant="outline" className="text-sm px-3 py-1 bg-background">
                  {wireSuggested} mm²
                </Badge>
              </div>
              <p className="text-xs text-muted-foreground mt-2">
                * Lưu ý: Tiết diện dây và định mức CB tính toán ở mức cơ bản (~125% In). Thực tế cần đối chiếu với điều kiện môi trường và phương pháp lắp đặt.
              </p>
            </CardContent>
          </Card>
        </div>
